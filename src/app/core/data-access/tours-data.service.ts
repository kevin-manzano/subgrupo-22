...
import toursJson from '../../../../public/data/tours.json';
import detailsJson from '../../../../public/data/tour-details.json';
...
export class ToursDataService {
  ...
  private readonly toursFavoritasCacheSubject = new BehaviorSubject<Map<number, boolean>>(
    new Map(),
  );
  ...
  getTourDetails(): Observable<TourDetail[]> {
    if (isPlatformServer(this.platformId)) {
      return of(detailsJson as TourDetail[]);
    }
    return this.http.get<TourDetail[]>(this.detailsPath);
  }

  // ... resto igual, no cambia nada más
  ...
  getTourWithDetailById(id: number): Observable<TourWithDetail | undefined> {
    return this.getTourById(id).pipe(
      switchMap((tour) => {
        if (!tour) return of(undefined);
        return this.getTourDetailByTourId(tour.id).pipe(
          map((detail) => (!detail ? undefined : { tour, detail })),
        );
      }),
    );
  }
